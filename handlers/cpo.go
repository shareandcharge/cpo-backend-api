...
func CpoCreateReimbursement(c *gin.Context) {
	...
	htmlTemplateRaw = strings.Replace(htmlTemplateRaw, "{{subTotal}}", fmt.Sprintf("%.4f", float64(reimb.Amount)*0.001), 1)
	htmlTemplateRaw = strings.Replace(htmlTemplateRaw, "{{totalVat}}", fmt.Sprintf("%.4f", float64(reimb.Amount)*0.001*0.2), 1)
	htmlTemplateRaw = strings.Replace(htmlTemplateRaw, "{{totalAmount}}", "£ "+strconv.Itoa(int(float64(reimb.Amount)*0.001*float64(1.20))), 1)
