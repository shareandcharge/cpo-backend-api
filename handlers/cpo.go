...
func CpoCreateReimbursement(c *gin.Context) {
	...
	htmlTemplateRaw = strings.Replace(htmlTemplateRaw, "{{total}}", fmt.Sprintf("£ %.4f", float64(reimb.Amount)*0.01), 1)
	htmlTemplateRaw = strings.Replace(htmlTemplateRaw, "{{subTotal}}", fmt.Sprintf("£ %.4f", float64(reimb.Amount)*0.01), 1)
	htmlTemplateRaw = strings.Replace(htmlTemplateRaw, "{{totalVat}}", fmt.Sprintf("£ %.4f", float64(reimb.Amount)*0.01*0.2), 1)
	htmlTemplateRaw = strings.Replace(htmlTemplateRaw, "{{totalAmount}}", fmt.Sprintf("£ %.4f", float64(reimb.Amount)*0.01*1.2), 1)
